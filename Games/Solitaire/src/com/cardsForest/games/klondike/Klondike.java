...
public class Klondike extends GameLogic {
    ...
    private void createStacks() {
        ...
        waste.getSprite().setRelativePos(0.2, 0.05);
        waste.setBehavior(new WasteBehavior(foundation));
        stacks.add(waste);

        //stock
        stock = Stack.newEmptyStack();
        stock.getSprite().setRelativePos(0.1, 0.05);
        ...
        for (int i = 0; i < foundation.length; i++) {
            ...
            s.getSprite().setRelativePos(0.6 + 0.1 * i, 0.05);
            s.setBehavior(b);
            foundation[i] = s;
            stacks.add(s);
        }
        ...
        for (int i = 0; i < tableau.length; i++) {
            ...
            s.getSprite().setRelativePos(0.2 + 0.1 * i, 0.4);
